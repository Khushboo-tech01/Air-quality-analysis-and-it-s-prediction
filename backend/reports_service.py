...
def build_prediction_pdf(prediction: Dict, user_email: str) -> bytes:
    ...
    if metrics:
        story.append(Paragraph("Model Metrics", styles["Section"]))
        story.append(_basic_table([
            ["Algorithm", str(metrics.get("algorithm", "-"))],
            ["RMSE", str(metrics.get("rmse", "-"))],
            ["MAE", str(metrics.get("mae", "-"))],
            ["R2", str(metrics.get("r2_score", metrics.get("r2", "-")))],
            ["MAPE", str(metrics.get("mape", "-"))],
            ["Model Version", str(metrics.get("model_version", "-"))],
            ["Dataset Size", str(metrics.get("dataset_size", "-"))],
        ], [55 * mm, 115 * mm]))
    ...
    if feature_importance:
        ...
        rows.extend([
            [str(item.get("feature", "-")), f"{float(item.get('importance', 0)) * 100:.1f}%"]
